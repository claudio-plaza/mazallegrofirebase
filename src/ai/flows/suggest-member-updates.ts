  ...
  suggestions: z.array(
    z.object({
      ...
      updateType: z
        .string()
        .describe(
          'The type of update suggested, e.g., medical_evaluation_expired, account_inactivity, etc.'
        ),
      reason: z.string().describe('The reason for the suggested update.'),
      urgency: z
        .string()
        .describe(
          'The urgency of the suggested update, e.g., high, medium, low.'
        ),
    })
  ).describe('An array of suggested member updates.'),
});
